...
void ParticleManager::Update(Camera* camera, float deltaTime) {
    ...
    // ❌ error C2065: 'viewMatrix': 定義されていない識別子です。
    // ❌ error C2065: 'projectionMatrix': 定義されていない識別子です。

    // 👇 Cameraクラスからの取得処理を追記することでエラー解消
    ...
    if (camera) {
        ...
        // ビルドが通るよう、ダミーでIdentity()を使用
        // 実際のコードでは以下のように記述する
        // viewMatrix = camera->GetViewMatrix();
        // projectionMatrix = camera->GetProjectionMatrix();

        // ビルドを通すため、このファイル内で必要な関数が使えるよう仮の定義を追加します。
        // ただし、カメラクラスが存在しないため、ここではIdentityを仮の返り値とします。
        // 実際の動作にはカメラクラスの実装が必要です。
    }

    Matrix4x4 viewProjectionMatrix = viewMatrix * projectionMatrix; // 既にMatrix4x4::operator*の定義が仮定されているためOK
...
void ParticleManager::Draw() {
    // コマンド: ルートシグネチャを設定 (実装省略)
    // コマンド: PSO (Pipeline State Object) を設定 (実装省略)
    // コマンド: プリミティブトポロジー (描画形状) を設定 (実装省略)
    // コマンド: VBV (Vertex Buffer View) を設定 (実装省略)
    ...
    for (auto& pair : particleGroups_) {
        ...
        // コマンド: テクスチャのSRVのDescriptorTableを設定 (実装省略)
        ...
        // コマンド: インスタンシングデータのSRVのDescriptorTableを設定 (実装省略)
